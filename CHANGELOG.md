...
- Spring placeholders that reference a property of the **same** `application*.yaml`
  are recognized as valid references: they are highlighted as
  matched instead of missing, and they resolve to the key that declares them. 
- Such a placeholder is navigable: *Go to Declaration* (Ctrl/Cmd+B) and Ctrl/Cmd+Click jump to the
  declaring key, because the same-file targets are contributed as real PSI references next to the
  Helm ones.
- Same-file targets are counted by the `N refs` tag and listed in its tooltip, and clicking the tag
  offers them alongside the Helm counterparts. The tooltip drops the `Helm` / `Spring` qualifier
  when both kinds of target are mixed.
